...
- **法律文案与现实一致**（App Store 5.1.1）：以 iOS 仓当前 `docs/legal/privacy-policy.{zh-Hans,en}.md` 与 spec 003 为真源。首启“开始”不授予许可，首次实际在线问事须具名 DeepSeek、披露字段与目的并征得许可；相同披露版本不重复，接收方、字段或目的实质变化须更新披露并重新许可。撤回在 App 的“我的 → 隐私与数据 → 隐私政策”页底。后端与第三方留存边界依当前政策，不能把历史训练退出事实升级为永久保证。此口径取代 2026-07-11 的零点名与独立改进开关表述。
